...
namespace VideoWeb.Controllers
{
        ...
        : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ConferenceForHostResponse>>> GetConferencesForHostAsync()
        {
            ...
            try
            {
                var conferenceForHostResponseMapper = mapperFactory
                    .Get<ConfirmedHearingsTodayResponse, List<HostConference>, ConferenceForHostResponse>();
                var username = User.Identity!.Name;
                var hearings = await bookingApiClient.GetConfirmedHearingsByUsernameForTodayAsync(username);
                var conferencesForHost = await videoApiClient.GetConferencesForHostByHearingRefIdAsync(
        ...
        public async Task<ActionResult<List<ConferenceForHostResponse>>> GetConferencesForStaffMemberAsync(
            [FromQuery] IEnumerable<string> hearingVenueNames)
        {
            logger.LogDebug("GetConferencesForStaffMember");
            
            try
            {
                var conferenceForHostResponseMapper = mapperFactory.Get<HostConference, ConferenceForHostResponse>();
                var hearingsForToday = await bookingApiClient.GetHearingsForTodayByVenueAsync(hearingVenueNames);
                var request = new GetConferencesByHearingIdsRequest
                    { HearingRefIds = hearingsForToday.Select(x => x.Id).ToArray() };
                var conferencesForStaffMember = await videoApiClient.GetConferencesForHostByHearingRefIdAsync(request);
            ...
            try
            {
                ...
                if (IsQuicklinkUser())
                {
                    var conferencesForIndividual =
                        await videoApiClient.GetConferencesTodayForIndividualByUsernameAsync(username);
                    var conferenceForIndividualResponseMapper =
                        mapperFactory.Get<IndividualConference, ConferenceForIndividualResponse>();
                ...
                else
                {
                    ...
                    var conferencesForIndividual =
                        await videoApiClient.GetConferencesTodayForIndividualByUsernameAsync(username);
                    var conferenceForIndividualResponseMapper = mapperFactory
                        .Get<ConfirmedHearingsTodayResponse, List<IndividualConference>,
            ...
            [FromQuery] VhoConferenceFilterQuery query)
        {
            ...
            try
            {
                var hearingsForToday = await bookingApiClient.GetHearingsForTodayByVenueAsync(query.HearingVenueNames);
                var request = new GetConferencesByHearingIdsRequest
                ...
                var allocatedHearings =
                    await bookingApiClient.GetAllocationsForHearingsAsync(conferences.Select(e => e.HearingRefId));
                var conferenceForVhOfficerResponseMapper = mapperFactory
                ...
                var responses = conferences
                    ...
                    .Select(x => conferenceForVhOfficerResponseMapper.Map(x,
                        allocatedHearings?.FirstOrDefault(conference => conference.HearingId == x.HearingRefId)))
        ...
        public async Task<ActionResult<ConferenceResponseVho>> GetConferenceByIdVhoAsync(Guid conferenceId)
        {
            ...
            try
            {
                conference = await videoApiClient.GetConferenceDetailsByIdAsync(conferenceId);
                if (conference == null)
                {
            ...
            var conferenceResponseVhoMapper = mapperFactory.Get<ConferenceDetailsResponse, ConferenceResponseVho>();
        ...
        public async Task<ActionResult<ConferenceResponse>> GetConferenceByIdAsync(Guid conferenceId)
        {
            ...
            var claimsPrincipalToUserProfileResponseMapper = mapperFactory.Get<ClaimsPrincipal, UserProfileResponse>();
            ...
            if (!userProfile.Roles.Contains(Role.StaffMember) &&
                (conference.Participants.TrueForAll(x => x.Username.ToLower().Trim() != username) ||
                 !conference.IsWaitingRoomOpen))
            {
                logger.LogInformation(
            ...
            var conferenceResponseMapper = mapperFactory.Get<Conference, ConferenceResponse>();
