...
(function (scope) {
    ...
    var HeartbeatFactory = function (pexrtc, cmdurl, hearingId, participantId, token, callback, delay, interval) {
        if (heartbeatFactoryInstance !== undefined) {
            heartbeatFactoryInstance.hearingId = hearingId;
            heartbeatFactoryInstance.participantId = participantId;
            heartbeatFactoryInstance.cmdurl = cmdurl;
            heartbeatFactoryInstance.token = token;
            heartbeatFactoryInstance.callback = callback;
    ...
    HeartbeatFactory.prototype.generateHeartbeat = function () {
        ...
        mediaStatistics.outgoing.audio['percentage-lost-recent'] =
            this.getRecentPercentageLost(this.lookbackValues.outgoing.audio).toFixed(1) + '%';
        mediaStatistics.outgoing.video['percentage-lost-recent'] =
            this.getRecentPercentageLost(this.lookbackValues.outgoing.video).toFixed(1) + '%';
        mediaStatistics.incoming.audio['percentage-lost-recent'] =
            this.getRecentPercentageLost(this.lookbackValues.incoming.audio).toFixed(1) + '%';
        mediaStatistics.incoming.video['percentage-lost-recent'] =
            this.getRecentPercentageLost(this.lookbackValues.incoming.video).toFixed(1) + '%';
