...
export class UpdateConferenceEndpointsRequest implements IUpdateConferenceEndpointsRequest {
    ...
    init(_data?: any) {
        if (_data) {
            if (Array.isArray(_data["existing_endpoints"])) {
                ...
                for (let item of _data["existing_endpoints"])
                    this.existing_endpoints!.push(EndpointResponse.fromJS(item));
            }
            if (Array.isArray(_data["new_endpoints"])) {
                this.new_endpoints = [] as any;
                for (let item of _data["new_endpoints"])
                    this.new_endpoints!.push(EndpointResponse.fromJS(item));
            }
            if (Array.isArray(_data["removed_endpoints"])) {
                this.removed_endpoints = [] as any;
                for (let item of _data["removed_endpoints"])
                    this.removed_endpoints!.push(item);
            }
        }
    }
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.existing_endpoints)) {
            ...
            for (let item of this.existing_endpoints)
                data["existing_endpoints"].push(item.toJSON());
        }
        if (Array.isArray(this.new_endpoints)) {
            data["new_endpoints"] = [];
            for (let item of this.new_endpoints)
                data["new_endpoints"].push(item.toJSON());
        }
        if (Array.isArray(this.removed_endpoints)) {
            data["removed_endpoints"] = [];
            for (let item of this.removed_endpoints)
                data["removed_endpoints"].push(item);
        }
        return data;
    }
}
