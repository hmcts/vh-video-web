...
import { UserProfileResponse, UserRole, ConferenceStatus, ConferenceEventRequest } from 'src/app/services/clients/api-client';
...
describe('BetaBannerComponent',
  () => {
    ...
    it('navigate to exit survey should see exit survey url in router url',
      fakeAsync(() => {
        fixture.ngZone.run(() => {
          ...
          tick();
          expect(router.url).toBe('/sub-component1');
          expect(component.pageUrl).toBe('https://www.smartsurvey.co.uk/s/VideoHearings_ExitSurvey/?pageurl=/sub-component1');
        });
      }));
  });
