...
export class SelfTestComponent implements OnInit, OnDestroy {
  ...
  setupTestAndCall(): void {
    ...
    this.subscription = this.videoWebService.getToken(this.selfTestParticipantId).subscribe((token: TokenResponse) => {
      ...
      this.logger.info(`Self test : Conference ${this.conference.id} | setupTestAndCall for Participant Id : ${this.participant.id} | Participant : ${this.videoWebService.getObfuscatedName(this.participant.display_name)}`);
      this.token = token;
      this.call();
    },
      (error) => {
  ...
  async retrieveSelfTestScore() {
    ...
    try {
      if (this.conference) {
        this.logger.info(`Self test : ConferenceId : ${this.conference.id} | retrieveSelfTestScore for Participant Id : ${this.participant.id} | Participant : ${this.videoWebService.getObfuscatedName(this.participant.display_name)}`);
        this.testCallResult = await this.videoWebService.getTestCallScore(this.conference.id, this.selfTestParticipantId).toPromise();
      } else {
        this.logger.info(`Self test : retrieveSelfTestScore for Participant Id : ${this.participant.id} | Participant : ${this.videoWebService.getObfuscatedName(this.participant.display_name)}`);
