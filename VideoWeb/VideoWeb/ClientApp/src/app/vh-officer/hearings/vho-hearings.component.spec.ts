...
describe('VhoHearingsComponent Filter', () => {
    ...
    it('should apply filter with selected status and location to conferences records', () => {
        ...
        expect(component.conferences[0].hearing_venue_name)
            .toBe(filter.locations[1].Description);
        expect(component.conferences[1].hearing_venue_name)
            .toBe(filter.locations[1].Description);
        expect(component.conferences[0].status)
            .toBe(filter.statuses[0].Status);
        expect(component.conferences[1].status)
            .toBe(filter.statuses[0].Status);
    });
