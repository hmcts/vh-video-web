...
//            audioLink.AudioFileLinks.First().ToLower().Should().Contain(_c.Test.NewHearingId.ToString().ToLower());
        ...
        public void ThenTheJudgeCanSeeInterpreterAndInterpreteeOnParticipantList()
        {
            ...
            var sharedDisplayName  = _browsers[_c.CurrentUser].TextOf(HearingRoomPage.InterpreteeName(interpretee.DisplayName));
            sharedDisplayName .Should().Contain($"{interpreter.DisplayName}");
        }
