...
{
    public class SuspendHearingTests : ConferenceManagementControllerTestBase
    {
        [SetUp]
        public void Setup()
        {
        ...
        public async Task should_return_accepted()
        {
            ...
            _mocker.Mock<IVideoApiClient>().Verify(
                x => x.SuspendHearingAsync(TestConference.Id, It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}
